# Schema — Data Model Contract

The **shared, authoritative data model** for Bonum, honored by every target (web, mobile) and by
every backend (mock SQLite, Sereus/Quereus). This is the concrete contract: entities, fields,
relationships, and the invariants each must uphold.

For the *why* behind these choices (balance-sheet philosophy, double-entry rationale, multi-unit
strategy), see [docs/Schema.md](../../../docs/Schema.md) and [docs/Vision.md](../../../docs/Vision.md).
Storage/adapter mapping (SQL DDL, CRDT shapes) belongs in generated consolidations, not here.
...
The shared **AccountGroup taxonomy** (the five types, seed hierarchy, and its rules) is documented
separately in [account-groups.md](./account-groups.md).
...
A distinct financial entity for which separate books are kept. This could be a business, a personal
household, a nonprofit, or any unit requiring its own set of books.
...
A classification grouping for accounts, shared across all entities. See
[account-groups.md](./account-groups.md) for the taxonomy, seed hierarchy, and rules.
...
**Constraint:** Transactions cannot be posted directly to an AccountGroup; they must target a
specific Account.
...
**Per-entity categorization:** Users who want custom account groupings within an entity (beyond the
global AccountGroups) can use parent accounts for this purpose. For example, create "Current Assets"
and "Fixed Assets" as parent accounts under the global "Assets" group, then nest specific accounts
beneath them.

**Inter-Entity Linking:**
- `linkedAccountId` connects counterpart accounts across entities (e.g., "Due to Entity B" ↔ "Due
  from Entity A")
...
A vendor, customer, or other external party. Partners are global and can link to multiple accounts
across entities.
...
**Note:** A partner can link to multiple accounts (AR, AP, across entities). The `type` field is
advisory — the actual relationship is defined by which accounts link to the partner.
...
See [docs/STATUS.md](../../../docs/STATUS.md) for unresolved schema decisions.
