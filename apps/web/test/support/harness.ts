...
/** True if a fixture is present. `tmp/` is gitignored (generated by scripts/gen-books.mjs), so larger
 *  fixtures may be absent on a fresh checkout — tripwire tests skip rather than fail when so. */
export function fixtureExists(name: string): boolean {
  return existsSync(resolve(repoRoot, `tmp/${name}`));
}
